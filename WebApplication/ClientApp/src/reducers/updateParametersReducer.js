...
import parameterActionTypes, { compareParamaters } from "../actions/parametersActions";
    ...
    switch(action.type) {
        case parameterActionTypes.PARAMETERS_UPDATED: {
            ...
            if(prevState) {
                ...
                paramSet = action.parameters.map( (param) => param?.readonly === true || compareParamaters(param, prevState.find((elem) => elem.name == param.name)) ? param : { ...param, changedOnUpdate: true } );
            }
