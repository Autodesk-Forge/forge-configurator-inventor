...
(Get-Content -path .\AWS-CICD\build-specs\appsettings.Local.json -Raw) -replace '<YOUR_CLIENT_ID>',"$FORGE_CLIENT_ID" -replace '<YOUR_CLIENT_SECRET>',"$FORGE_CLIENT_SECRET" -replace '<BUCKET_KEY_SUFFIX>',"$Env:CODEBUILD_BUILD_NUMBER" | Set-Content -Path webapplication\appsettings.Local.json
