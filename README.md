...
### UI Tests
* For UI tests we are using `CodeCeptJs` framework. All tests are stored in `ClinetApp/src/__tests__/` and we filter all files end with `*_test.js`. 
1. From Visual Studio Code
    * For all UI tests Run this command: `npx codeceptjs run`
    * For particular file you can use this command: `npx codeceptjs run src/__tests__/<test file name>`
